...
class DepthObstacleCloudNode(Node):
    ...
    def depth_callback(self, msg):
        points = self.depth_to_obstacle_points(msg)
        cloud = self.make_cloud(msg.header.stamp, points)
        self.publisher.publish(cloud)
        self.log_status(len(points))

    def depth_to_obstacle_points(self, msg):
        ...
        camera_x_offset = float(self.get_parameter('camera_x_offset_m').value)
        x_base = ground_forward[obstacle] + camera_x_offset
        y_base = -camera_x_right[obstacle]
        z_base = height_above_floor[obstacle]
        points = np.column_stack((x_base, y_base, z_base)).astype(np.float32)
    ...
    def make_cloud(self, stamp, points):
        msg = PointCloud2()
    ...
    def log_status(self, point_count):
        now = self.get_clock().now()
        if (now - self.last_log_time).nanoseconds < 1_000_000_000:
            return
        self.last_log_time = now
        self.get_logger().info(
            f'depth obstacle cloud points={point_count}'
        )
